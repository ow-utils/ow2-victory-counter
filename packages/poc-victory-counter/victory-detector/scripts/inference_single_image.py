...
from victory_detector.inference.predictor import VictoryPredictor


def main():
    parser = argparse.ArgumentParser(description="単発画像推論")
    parser.add_argument("--image", type=Path, required=True, help="入力画像パス")
    parser.add_argument("--model", type=Path, required=True, help="モデルファイルパス")
