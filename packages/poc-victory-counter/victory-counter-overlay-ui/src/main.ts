...
import { apiClient, type StateResponse, type AdjustmentPayload } from "./apiClient";
import { mockTimeline, summarizeEvents, type TimelineEvent } from "./mockData";

type Summary = ReturnType<typeof summarizeEvents>;
...
const outcomeSelect = document.getElementById("adjust-outcome") as HTMLSelectElement | null;
const deltaInput = document.getElementById("adjust-delta") as HTMLInputElement | null;
...
const flattenEvents = (payload: StateResponse): TimelineEvent[] => {
...
const updateFromServer = async (): Promise<void> => {
  try {
...
const startPolling = (): void => {
  ...
  const duration = Number.isFinite(interval) ? interval : 5000;
  setInterval(() => {
    void updateFromServer();
  }, duration);
};
...
const getAdjustPayload = () => {
  const value = (outcomeSelect?.value as AdjustmentPayload["value"]) ?? "victory";
...
const handleAdjustSubmit = async (event: SubmitEvent): Promise<void> => {
  ...
  try {
    ...
    state.status = `Adjustment failed: ${(error as Error)?.message ?? "unknown error"}`;
    render();
  }
};
