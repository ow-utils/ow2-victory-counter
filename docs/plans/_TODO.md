...
- [x] 連続検知機能の実装（2025-11-10）
  - 連続N回（デフォルト2回）同じ結果が出た場合のみカウント
  - キャプチャ間隔を0.25秒に短縮（0.5秒で判定確定）
...
- [x] StateManager に状態管理機能を追加（`_cooldown_state`）
- [x] `_none_consecutive_count`, `_none_required_consecutive` パラメータを追加
