...
**背景**:
- victory_progressbar や defeat_progressbar の画面のまま放置できる
- 放置中にクールダウンが明けると、同じ画面を再検知して誤カウントされる

**対処方法（2段階クールダウン）**:
- **第1段階**: クールダウン時間（180秒）経過
- **第2段階**: none/unknown が規定回数（デフォルト50回）連続検知
- 両方の条件を満たして初めて、次の勝敗判定が可能になる

**状態遷移**:
- `COOLDOWN` → すべての検知を無視（0〜180秒）
- `WAITING_FOR_NONE` → none をカウント、victory/defeat は無視（180秒以降）
- `READY` → 次の勝敗判定が可能（none 50回後）

**実施タスク**:
- [ ] StateManager に状態管理機能を追加（`_cooldown_state`）
- [ ] `_none_consecutive_count`, `_none_required_consecutive` パラメータを追加
- [ ] `record_detection()` メソッドで2段階クールダウンロジックを実装
- [ ] 既存の連続検知機能（victory/defeat 3回）との整合性確保
- [ ] architecture.md に仕様を追記
- [ ] 実運用テストと動作確認
