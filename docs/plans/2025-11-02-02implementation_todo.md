...
- [x] `fetch('/state')` のポーリング処理とタイムアウト/リトライを実装する
- [x] ローカルの HTTP サーバと接続し、リアルタイムでカウントが反映されることを確認する
- [x] 管理用 UI もしくはツールから `POST /adjust` を実行し、補正が反映されることを検証する
- [x] ネットワーク障害を想定したエラーハンドリングと表示を実装する

## フェーズ6: OBS 連携

- [x] OBS Scripts Manager に `victory-detector` を読み込むスクリプトと手順を整備する
- [x] OBS ブラウザソースにオーバーレイを適用する手順をまとめ、リアルタイム連携を検証できる状態にする
...
- [ ] OBS キャプチャ画像から Victory/Defeat バナー等を自動検出する具体的な実装計画（テンプレートマッチング・色抽出・OCR 等）を立案し、スタブ処理を実装に置き換える
