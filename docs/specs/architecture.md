...
- **none 連続検知による解除条件**：
  - クールダウンが明けても、`victory_progressbar` / `defeat_progressbar` 画面のまま放置すると勝敗が再カウントされてしまう
  - これを防ぐため、`required_none_consecutive`（デフォルト30回）だけ `none` クラスを連続検知してからでないと勝敗カウントが解禁されない
  - `none` は勝敗バナーが表示されていない通常シーンを意味し、新しい試合が開始されたことを示すシグナルとして扱う
...
- `required_none_consecutive` (デフォルト: 30): クールダウン解除後に必要な `none` 連続検知回数。`0` を指定すると無効化できる
