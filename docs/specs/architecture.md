...
231秒:  victory 検知 (1/2) → 連続カウント開始
232秒:  victory 検知 (2/2) → 確定、COOLDOWN へ
```
...
```
0.00秒: victory検知 (1/2) → ログなし、スクリーンショット保存
0.25秒: victory検知 (2/2) → カウント確定、イベントログ記録
0.50秒: victory検知 (cooldown) → クールダウン中、カウントしない
```

```
0.00秒: victory検知 (1/2) → ログなし、スクリーンショット保存
0.25秒: defeat検知 (1/2) → リセット、スクリーンショット保存
0.50秒: defeat検知 (2/2) → カウント確定
```

### パラメータ

- `required_consecutive` (デフォルト: 2): カウントに必要な連続検知回数
